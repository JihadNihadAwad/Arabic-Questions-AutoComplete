...
PORT=5001
CHECKPOINT_PATH = 'checkpoints_last_ann_embedding_dim_256_hidden_dim_512_num_layers_3_epochs_100/autocomplete_model_epoch_100.pth'  
...
model = AutocompleteANNModel(processor.vocab_size, embedding_dim=embedding_dim, 
...
def predict():
    ...
    predictions = prediction_manager.predict_next_word(text, top_k=num_predictions)
    output = [{'word': word, 'probability': prob} for word, prob in predictions]

    return jsonify({'model': 'ann', 'output': output})
